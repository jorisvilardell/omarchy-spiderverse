.pragma library


var background = "#160a2e";
var darkBackground = "#0d0620";
var darkerBackground = "#070312";
var lighterBackground = "#2d1b4e";

var foreground = "#f2e9ff";
var darkForeground = "#9a86c2";
var lightForeground = "#d8c9f5";
var brightForeground = "#ffffff";

var accent = "#ff2d95";
var selection = "#4a2e7a";
var muted = "#6b5390";

var red = "#df1f2d";
var yellow = "#ffd23f";
var orange = "#ff6b4a";
var green = "#3ef2a0";
var cyan = "#00e5ff";
var blue = "#447bbe";
var magenta = "#ff2d95";
var brown = "#8a5a6b";

var brightRed = "#f4143c";
var brightYellow = "#ffe066";
var brightGreen = "#5cffb0";
var brightCyan = "#4de8ff";
var brightBlue = "#6fa8ff";
var brightMagenta = "#ff6ec7";

var categoryTints = {
    "terminal": cyan,
    "éditeur": green,
    "editeur": green,
    "web": blue,
    "média": green,
    "media": green,
    "notes": magenta,
    "social": yellow,
    "système": darkForeground,
    "systeme": darkForeground,
    "dev": magenta,
    "jeux": blue,
    "design": magenta,
    "mail": blue,
    "lecture": darkForeground,
    "réseau": cyan,
    "reseau": cyan
};

var tintCycle = [magenta, cyan, green, yellow, blue, brightMagenta];

function tintFor(category, index) {
    var key = String(category || "").toLowerCase();
    if (categoryTints[key]) return categoryTints[key];
    return tintCycle[index % tintCycle.length];
}

function initials(name) {
    var clean = String(name || "").replace(/[^A-Za-zÀ-ÿ]/g, "");
    return clean.slice(0, 2).toUpperCase();
}
